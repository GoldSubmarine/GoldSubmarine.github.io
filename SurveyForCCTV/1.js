...
window.onload = function () {
	...
	function autoPlay() {
		...
		oPicList.addEventListener('touchend', function (ev) {
			ev = ev.changedTouches[0];
			var dix = Math.abs(ev.pageX - iStart);
			var onoff = dix > aLi[0].offsetWidth/4;
			if(onoff){
			...
			tranX = -iNow * 640;
			oPicList.style.transition = '0.3s';
			oPicList.style.WebkitTransform = 'translateX(' + tranX + 'px)';
			for (var i = 0; i < aLi.length; i++) {
				aNav[i].className = '';
			}
	...
	function star() {
		...
		var _loop = function _loop(i) {
			var aNav = aLi[i].getElementsByTagName('a');
